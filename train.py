...
from stolen_weights_utils import load_stolen_weights
...
if not (os.path.exists(train_bin) and os.path.exists(val_bin)):
    raise FileNotFoundError(f"Missing train.bin/val.bin in {data_dir}")
...
if init_from == 'scratch':
    ...
    model = GPT(gptconf)
    if stolen_ckpt_path != "":
        print("Training using stolen weights")
        load_stolen_weights(model, stolen_ckpt_path)
    else:
